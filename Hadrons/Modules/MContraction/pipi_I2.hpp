...
void TPiPi<FImpl1, FImpl2, FImpl3, FImpl4>::execute(void)
{
    ...
    auto efficientTmp0 = adj(q1)*q2;
    auto efficientTmp1 = adj(q3)*q4;

    auto s0 = sink1(efficientTmp0);
    auto s1 = sink2(efficientTmp1);
    //auto s2 = sink1(efficientTmp1);
    //auto s3 = sink2(efficientTmp0);

    for(size_t t=0; t<result.corr.size(); ++t)
    {
        auto d0 = trace(s0[t]*s0[t]);
        auto d1 = trace(s0[t])*trace(s0[t]);
